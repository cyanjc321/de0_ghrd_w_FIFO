...
QUARTUS_BASE := $(basename $(QUARTUS_QSF))
...
QUARTUS_SOF := output_files/$(patsubst %.qsf,%.sof,$(QUARTUS_QSF))
...
$(QUARTUS_PIN_ASSIGNMENTS_STAMP): $(QSYS_STAMP)
	@for pin_assign_tcl_file in $$(find $(QSYS_BASE) -name '*_pin_assignments.tcl'); do \
		quartus_map $(QUARTUS_QSF); \
		echo "Applying $${pin_assign_tcl_file} to $(QUARTUS_QSF)..."; \
		quartus_sta -t $${pin_assign_tcl_file} $(QUARTUS_QSF); \
	done
	$(stamp_target)
...
$(QUARTUS_STAMP): $(QUARTUS_DEPS)
	...
	quartus_sh --flow compile $(QUARTUS_QSF)
	$(stamp_target)
